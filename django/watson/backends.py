        ...
        return queryset.extra(
            ...
            where=(
                ...
                "watson_searchentry.{ref_name} = {table_name}.{pk_name}{ref_name_typecast}".format(
                    ...
                    pk_name=connection.ops.quote_name(
                        pk.db_column or pk.attname),
                    ref_name_typecast=ref_name_typecast
                ),
                "watson_searchentry.content_type_id = %s"
            ),
            params=(engine_slug, self.escape_postgres_query(
                search_text), content_type.id),
        )
...
class MySQLSearchBackend(SearchBackend):
    ...
    def do_install(self):
        ...
        cursor.execute(
            "CREATE FULLTEXT INDEX watson_searchentry_title ON watson_searchentry (title)")
        cursor.execute(
    ...
    def do_uninstall(self):
        ...
        cursor.execute(
            "DROP INDEX watson_searchentry_fulltext ON watson_searchentry")
        cursor.execute(
            "DROP INDEX watson_searchentry_title ON watson_searchentry")
        cursor.execute(
            "DROP INDEX watson_searchentry_description ON watson_searchentry")
        cursor.execute(
            "DROP INDEX watson_searchentry_content ON watson_searchentry")
    ...
    def do_filter(self, engine_slug, queryset, search_text):
        ...
        return queryset.extra(
            ...
            params=(engine_slug, self._format_query(
                search_text), content_type.id),
        )
