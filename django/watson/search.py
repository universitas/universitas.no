...
class SearchEngine(object):
    ...
    def register(self, model, adapter_cls=SearchAdapter, **field_overrides):
        ...
        if field_overrides:
            # Conversion to str is needed because Python 2 doesn't accept
            # unicode for class name
            adapter_cls = type(
            ...
            if isinstance(model, QuerySet):
                ...
                if has_int_pk(model):
                    ...
                    if live_ids:
                        ...
                        # This bit of nonsense ensures that this aspect of the
                        # query will be impossible to fulfill.
                        filter &= Q(
...
def get_backend(backend_name=None):
    ...
    if not backend_name:
        backend_name = getattr(settings, "WATSON_BACKEND",
                               "watson.backends.AdaptiveSearchBackend")
