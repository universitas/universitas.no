...
def get_engine(engine_slug_):
    ...
    try:
        ...
        raise CommandError(
            "Search Engine \"%s\" is not registered!" % force_text(engine_slug_))
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # get the search engine we'll be checking registered models for, may be
        # "default"
        ...
        for model_name in args:
            try:
                ...
                if len(matching_models) > 1:
                    raise CommandError(
                        "Model name \"%s\" is not unique, cannot continue!" % model_name)
                if matching_models:
                    model = matching_models[0]
                else:
                    model = None
            if model is None or not search_engine.is_registered(model):
                raise CommandError("Model \"%s\" is not registered with django-watson search engine \"%s\"!" %
        ...
        if models:  # request for (re-)building index for a subset of registered models
            ...
            if engine_selected:
                ...
                if verbosity >= 2:
                    ...
                    print("Rebuilding models registered with search engine \"%s\"" %
                          force_text(engine_slug))
            else:  # loop through all engines
                engine_slugs = [x[0]
                                for x in SearchEngine.get_created_engines()]
            ...
            for engine_slug in engine_slugs:
                ...
                # Clean out any search entries that exist for stale content
                # types. Only do it during full rebuild
                valid_content_types = [ContentType.objects.get_for_model(
